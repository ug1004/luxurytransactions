...
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"Args":["Health_register","H00001","BSTUDENT","20221001","J00","KCM", "20221208"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3
...
set -x
peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["Health_query","H00001"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt