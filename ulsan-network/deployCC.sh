...
CC_NAME="goods"
CC_SRC_PATH="../contract/goods/v0.9"
CC_RUNTIME_LANGUAGE="golang"
CC_VERSION="0.9"
CHANNEL_NAME="ulsanchannel"
...
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"Args":["Goods_register","S00001","BSTUDENT","HERMES-KANGNAM-20221202"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3
...
set -x
peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["Goods_history","S00001"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt